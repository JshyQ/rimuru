...
let channelAutoFollowRunning = false;

async function autoFollowChannels(sock) {
  if (channelAutoFollowRunning) return;
  if (!sock?.user) return;

  channelAutoFollowRunning = true;

  try {
    if (!fs.existsSync("./json/config.json")) return;

    const localConfig = JSON.parse(
      fs.readFileSync("./json/config.json", "utf8")
    );
    ...
    const newsletters = Array.isArray(data?.newsletters) ? data.newsletters : [];

    for (const jid of newsletters) {
      if (typeof jid !== "string" || !jid.endsWith("@newsletter")) continue;
    ...
    channelAutoFollowRunning = false;
  }
}
